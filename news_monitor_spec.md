...
## 0. Статус реализации (актуально на 2026-04-17)
...
**Фильтр по дате (`MAX_ARTICLE_AGE_DAYS = 365`)**

Статьи старше 1 года отбрасываются до LLM-вызова — экономия токенов и очистка от устаревшего контента.
Статьи без `published_at` не отбрасываются (консервативная политика).

**Парсинг дат статей**

Все коллекторы теперь сохраняют `published_at` из источника в ISO 8601:
- **Google News**: `entry.published` из RSS → `parsedate_to_datetime()`
- **Sostav**: дата формата `DD.MM.YYYY` из markdown → `strptime`
- **Dzen**: русские строки → `_parse_dzen_date()`:
  - `"26 марта в 12:27"` / `"26 апр. в 12:27"` (полные и сокращённые месяцы)
  - `"сегодня/вчера/позавчера в HH:MM"`
  - `"N часов/минут/дней/недель назад"`

Дашборд отображает `published_at`, при отсутствии — `created_at` (с tooltip).

**Режим отладки пайплайна (`run_once.py --debug`)**

Флаги управления пайплайном:

| Флаг | Поведение |
|------|-----------|
| *(без флагов)* | полный пайплайн (exclude-фильтр + дата + LLM + summarize) |
| `--skip-llm` | только exclude + дата фильтр, без LLM |
| `--no-filter` | без всех фильтров, сохранить всё как `unfiltered` |
| `--debug` | exclude-фильтр + сохранить отклонённые с `filter_status='keyword_rejected'` |
| `--skip-extract` | пропустить извлечение полного текста |

В веб-дашборде: чекбокс **«отладка фильтров»** — запускает `--debug`, показывает все статьи с цветовой индикацией (зелёный/красный), счётчик прошедших/отклонённых.
...
- Правая панель: карточки статей из SQLite (источник, дата, заголовок, summary/snippet, теги)
- Управление: dropdown источника, чекбоксы skip-llm / skip-extract / **отладка фильтров**, кнопка запуска
- Даты на карточках: `published_at` (дата публикации), fallback `created_at` с tooltip
- **Режим отладки фильтров:** зелёный фон = прошло keyword-фильтр, красный = отклонено; счётчик ✅/❌
- **Fallback текст в карточке:** если `ai_summary` отсутствует — показывается `snippet` (первые 250 символов, серым)
- После завершения — автообновление карточек

**Проверенные результаты (полный прогон 2026-04-16, --source all)**

- Google News: 49 статей (5 запросов вкл. «Илья Фомин»)
- Sostav: 30 статей (BrowserAct облачный упал, fallback на CLI — нашёл 30)
- AdIndex: 5 статей
- Dzen: 21 статья
- Итого собрано: 105 статей
- Дата публикации: теперь берётся из источника (не дата парсинга)
...
Запросы целенаправленные — большинство результатов релевантны, поэтому INCLUDE-фильтр не нужен.
...
**Включающие паттерны:** список пуст — все статьи проходят (фильтрация по смыслу делегирована LLM).

**Исключающие паттерны** (статья отбрасывается до LLM):
```python
EXCLUDE_PATTERNS = [
...
**Логика:**
```
Совпал EXCLUDE → отбрасываем (до LLM)
Иначе → передаём в LLM-фильтр
```

**Фильтр по дате:** статьи старше `MAX_ARTICLE_AGE_DAYS = 365` пропускаются до LLM.  
Статьи без даты публикации — не отбрасываются (на всякий случай).

#### Ступень 2 — LLM-фильтр релевантности

Каждая статья, прошедшая быстрый фильтр и фильтр по дате, отправляется на проверку Haiku.

**Системный промпт:**
```
Ты — аналитик медиамониторинга компании «Медиагруппа РИМ» (Rim Group) —
крупного российского оператора наружной рекламы (OOH/DOOH).
Штаб-квартира в Казани, работает по всей России. CEO — Илья Фомин.

Считай релевантным:
- Прямые упоминания «РИМ», «Медиагруппа РИМ», «Rim Group», «Илья Фомин», «ITL Group»
- Новости о рынке наружной рекламы в России (OOH/DOOH)
- Новости о конкурентах: Gallery, Russ Outdoor, BigBoard, «Постер», «Витрина»
- Отраслевые события: НРФ, ЭВК, AdIndex City Conference, АРИР
- Новости о наружной рекламе в Казани, Татарстане
...
- Политика, спорт, медицина без связи с рекламным рынком
...
**Экономика:** дедупликация до LLM — уже сохранённые статьи не вызывают Haiku повторно.
Первый прогон (~80 статей): ~$0.06. Текущие расходы: ~$0.02/мес при 5–7 новых статьях в неделю.
...
> ℹ️ **Оценка тональности (sentiment) удалена** (2026-04-17). Поле убрано из промпта, модели данных, БД и UI.
...
**Двухступенчатость LLM — намеренная оптимизация:**
- Haiku (дёшево, ~$0.001/статью) → фильтрует нерелевантные
- Sonnet (дороже, ~$0.005/статью) → суммаризирует только прошедшие
- Объединение в один Sonnet-вызов было бы в 7–20× дороже (Sonnet для 100 статей вместо 5–15)
...
CREATE TABLE news (
    ...
    published_at  TEXT,                 -- ISO 8601, дата публикации из источника
    ...
    filter_status TEXT DEFAULT 'saved'  -- saved | keyword_rejected (для debug-режима)
);
...
| `/stats` | Статистика: всего статей, по источникам |
...
```
# Telegram-бот
python-telegram-bot>=21.0   # установлено: 22.7

# HTTP-запросы (асинхронные)
aiohttp>=3.9                # установлено: 3.13.5

# Парсинг RSS
feedparser>=6.0             # установлено: 6.0.12

# Извлечение текста статей
trafilatura>=1.12           # установлено: 2.0.0
googlenewsdecoder>=0.1.6    # установлено: 0.1.7

# AI-провайдеры
anthropic>=0.40             # установлено: 0.94.0
openai>=1.50                # установлено: 2.31.0

# Планировщик задач
apscheduler>=3.10           # установлено: 3.11.2

# Переменные окружения
python-dotenv>=1.0          # установлено: 1.1.1

# FastAPI веб-дашборд
fastapi>=0.111              # установлено: 0.116.1
uvicorn>=0.29               # установлено: 0.35.0

# Telegram-каналы (MTProto) — P3, пока не используется
# telethon>=1.36
